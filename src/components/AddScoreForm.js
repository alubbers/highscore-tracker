  ...
  const validateForm = () => {
    ...
    if (formData.isNewPlayer) {
      if (!formData.playerName.trim()) {
        ...
        newErrors.playerName = 'Player name must be at least 2 characters long';
              ...
              <Form onSubmit={handleSubmit}>
                  ...
                  >
                    ...
                    {games.map(game => (
                      <option key={game.id} value={game.id}>
                        {game.name} {game.isTimeBased ? '(Time-Based)' : '(Score-Based)'}
                      </option>
                    ))}
                  </Form.Select>
                    ...
                    <div className="d-flex justify-content-between align-items-center">
                      <div>
                        ...
                        {selectedGame.description && (
                          <div className="small">{selectedGame.description}</div>
                        )}
                      </div>
                      <Badge bg={selectedGame.isTimeBased ? 'info' : 'success'}>
                        {selectedGame.isTimeBased ? 'Time-Based' : 'Score-Based'}
                      </Badge>
                    </div>
                  </Alert>
                )}
                  ...
                  <Form.Label>
                    ...
                    {selectedGame && (
                      <span className="text-muted">
                        {selectedGame.isTimeBased ? ' (in seconds)' : ' (points)'}
                      </span>
                    )}
                  </Form.Label>
                  <InputGroup>
                    <Form.Control
                      ...
                      step="0.01"
                      min="0"
                      placeholder={selectedGame?.isTimeBased ? "Enter time in seconds" : "Enter score"}
                      value={formData.scoreValue}
                      onChange={(e) => handleFieldChange('scoreValue', e.target.value)}
                      isInvalid={!!errors.scoreValue}
                    />
                    {formData.scoreValue && selectedGame && (
                      <InputGroup.Text>
                        {formatScorePreview(formData.scoreValue, selectedGame.isTimeBased)}
                      </InputGroup.Text>
                    )}
                  </InputGroup>
                  <Form.Control.Feedback type="invalid">
                ...
                <div className="d-grid gap-2 d-md-flex justify-content-md-end">
                  ...
                  <Button
                    variant="primary"
                    type="submit"
                    disabled={isSubmitting || !formData.gameId || !formData.scoreValue}
                  >
                    {isSubmitting ? (
                      <>
          ...
          <Card bg="light">
            <Card.Body>
              ...
              <ul className="mb-0">
                ...
                <li>For time-based games, enter times in seconds (e.g., 95.5 for 1:35.50)</li>
                <li>Create new players on the fly or select from existing ones</li>
                <li>Add notes to remember special achievements or conditions</li>
